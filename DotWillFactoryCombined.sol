...
/**
 * @title  DotWillFactory
 * @notice Deploys individual DotWill contracts for each user.
 *         One factory, unlimited wills.
 *
 * @dev    Deployed once on Polkadot Hub. Users call createWill() to get
 *         their own personal DotWill contract instance.
 */
contract DotWillFactory {

    // ─── State ────────────────────────────────────────────────────────────
    address public owner;

    // wallet address → their DotWill contract address
    mapping(address => address) public willOf;

    // all deployed wills (for enumeration / analytics)
    ...
    // ─── Errors ───────────────────────────────────────────────────────────
    error AlreadyHasWill(address existingWill);
    error NoWillFound();
    error NotOwner();
    ...
    /**
     * @notice Deploy a new DotWill contract for the caller.
     *         Each wallet can only have one will at a time.
     *
     * @param _beneficiaryWallets  Array of beneficiary addresses
     * @param _shareBps            Array of shares in basis points (must sum to 10000)
     * @param _labels              Array of human-readable labels
     * @param _heartbeatInterval   Seconds between required heartbeats
     * @param _gracePeriod         Extra seconds after missed interval
     */
    function createWill(
        address payable[] memory _beneficiaryWallets,
        uint256[]          memory _shareBps,
        ...
        // One will per wallet
        if (willOf[msg.sender] != address(0)) {
            revert AlreadyHasWill(willOf[msg.sender]);
        }

        // Deploy a fresh DotWill — pass msg.sender as the real owner
        // so the user (not this factory) controls the will contract
        DotWillV2 will = new DotWillV2{value: msg.value}(
            payable(msg.sender),  // _realOwner = the actual user
            _beneficiaryWallets,
            _shareBps,
            _labels,
            _heartbeatInterval,
        ...
        address willAddr = address(will);
        willOf[msg.sender] = willAddr;
        allWills.push(willAddr);

        emit WillCreated(msg.sender, willAddr, block.timestamp);
        return willAddr;
    }

    /**
     * @notice Returns the will contract address for a given user.
     *         Returns address(0) if user has no will.
     */
    ...
    /**
     * @notice Returns true if the given wallet has a deployed will.
     */
    function hasWill(address user) external view returns (bool) {
        return willOf[user] != address(0);
    }

    /**
     * @notice Total number of wills deployed through this factory.
     */
    ...
    /**
     * @notice Remove a will record (called when user revokes their will).
     *         Only the will contract itself can call this.
     */
    function removeWill(address user) external {
        require(msg.sender == willOf[user], "Not the will contract");
        delete willOf[user];
    }

    /**
     * @notice Factory owner can clear a stale record if needed.
     */
    function adminClear(address user) external {
        if (msg.sender != owner) revert NotOwner();
        delete willOf[user];
    }
}
